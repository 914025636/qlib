...
4. **收盘确认**：突破后的确认柱收盘靠近本根 K 线高点，而且上影线不能过长。

事件回测采用因果时序：第 $t$ 根 K 线发出突破信号，第 $t+1$ 根作为确认柱，第 $t+2$ 根开盘才成交，避免用确认柱收盘后的信息回到同一根 K 线成交。
...
$$
UW_{t+1}=\frac{High_{t+1}-\max(Open_{t+1},Close_{t+1})}
{High_{t+1}-Low_{t+1}+\epsilon}
$$

默认要求 $UW_{t+1}\leq0.4$，并要求确认柱的收盘位置至少为 $0.6$。如果确认柱冲高后大幅回落，即上影线过长或收盘位置过低，则不追入。
...
模型数据集仍使用第 $t$ 根 K 线生成特征，并以第 $t+1$ 根收盘价定义训练标签：
...
对于 `plot_backtest.py` 的形态事件回测，成交时序单独采用：

1. 第 $t$ 根收盘确认突破信号；
2. 第 $t+1$ 根收盘检查上影线、收盘位置和突破位；
3. 确认通过后，第 $t+2$ 根开盘成交；
4. 从成交价开始持有 `horizon_bars` 根 K 线。

因此，截图中如果第 $t+1$ 根已经出现很长上影线，就会被拒绝，而不会按该根收盘价追入。

## 7. BTC/USDT 1 分钟数据实证

数据范围为 2024-01-01 至 2026-08-30，共约 140 万根 1 分钟 K 线。下面的第一组数据是未加入确认柱过滤的原始事件研究，用于观察形态本身；事件回测的最终结果见后面的过滤后统计。
...
加入确认柱过滤后，`plot_backtest.py` 的测试集结果如下：

| 持有期 | 候选信号 | 确认拒绝 | 重叠跳过 | 实际成交 | 总收益 | 最大回撤 | 胜率 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 15 分钟 | 672 | 367 | 52 | 253 | -21.43% | -22.68% | 18.58% |
| 60 分钟 | 671 | 340 | 95 | 236 | -18.81% | -22.92% | 26.69% |

确认拒绝默认条件为：确认柱上影线/振幅不超过 40%，确认柱收盘位置不低于 60%。
...
加入确认柱过滤后的二值突破事件对照回测，使用相同的 1 分钟数据和 60 分钟持有期，共执行 `236` 笔交易，总收益约 `-18.81%`，同期买入持有约 `14.70%`，最大回撤约 `-22.92%`。相比旧版无确认过滤的 `500` 笔交易，交易数量和回撤明显下降，但结果仍未转正，说明过滤长上影线是风险控制改进，不等于策略已经有效。对照报告位于 `.qlib/experiments/squeeze_breakout_factor_h60/squeeze_breakout_backtest.html`。
...
Set-Location D:\Git\AIQuant\qlib
& D:\Git\AIQuant\.venv\Scripts\python.exe `
  examples\compare_binance_experiments.py `
  --output-dir .qlib\experiments\binance_experiment_comparison
```

页面同时展示模型回测和直接因子回测的净值、回撤、交易数、胜率、多头暴露、持仓统计，以及模型平均预测与交易成本门槛的关系。输出文件为 `.qlib\experiments\binance_experiment_comparison/binance_experiment_comparison.html`。
...
[`plot_backtest.py`](plot_backtest.py) 专门检验二值形态事件：第 $t$ 根发出信号，第 $t+1$ 根检查确认柱，确认通过后在第 $t+2$ 根开盘买入，默认持有 15 分钟后卖出；持仓期间出现的其他信号会被忽略。默认过滤确认柱上影线/振幅超过 40% 或收盘位置低于 60% 的信号。每边手续费为 0.04%，每边滑点为 0.01%。
...
| `squeeze_breakout_trades.csv` | 通过确认并实际成交的逐笔交易及因子值 |
...
HTML 报告包含策略与买入持有净值、回撤、价格与买卖点、确认拒绝标记、逐笔收益分布、评分和收益散点、月度复合收益、评分分位收益、代表交易 K 线及全部交易表。

当前 2026-04-01 至 2026-08-30 测试集、15 分钟持有期、加入确认柱过滤后的结果为：

- 候选信号 672 个，其中 367 个未通过确认柱过滤，52 个因持仓重叠跳过，实际执行 253 笔；
- 计入成本后总收益 -21.43%，最大回撤 -22.68%；
- 不计成本的逐笔复合收益约 +1.19%；
- 买入持有收益 +14.70%，交易胜率 18.58%。
